...
export default function ConfiguracionPage() {
  ...
  const [usoTokenDetalleId, setUsoTokenDetalleId] = useState<string | null>(null)
                              ...
                              : row.tipo_operacion === 'Compra'
                                ...
                                : row.tipo_operacion === 'Ajuste_manual'
                                  ...
                                  : row.tipo_operacion === 'WhatsApp'
                                    ? 'WhatsApp'
                                    : row.tipo_operacion === 'Consentimiento_sesion'
                                      ? 'Privacidad (LOPD) en sesión'
                                      : row.tipo_operacion
                      const detalle = [row.asamblea_nombre, row.conjunto_nombre].filter(Boolean).join(' · ') || '—'
                      const hayDetalleUnidades =
                        row.tipo_operacion === 'Consentimiento_sesion' &&
                        Array.isArray(row.unidad_labels) &&
                        row.unidad_labels.length > 0
                      const isOpen = usoTokenDetalleId === row.id
                      return (
                        <>
                        <tr
                          key={row.id}
                        ...
                        >
                          ...
                          <td className="py-3 px-4 text-gray-600 dark:text-gray-400 max-w-[220px]">
                            <div className="truncate" title={detalle}>{detalle}</div>
                            {hayDetalleUnidades && (
                              <button
                                type="button"
                                onClick={() => setUsoTokenDetalleId(isOpen ? null : row.id)}
                                className="mt-1 text-xs text-indigo-600 dark:text-indigo-400 hover:underline"
                              >
                                {isOpen ? 'Ocultar unidades' : 'Ver unidades en esta operación'}
                              </button>
                            )}
                          </td>
                        </tr>
                        {isOpen && hayDetalleUnidades && (
                          <tr key={`${row.id}-det`}>
                            <td colSpan={5} className="px-4 pb-3 pt-0 text-xs text-gray-600 dark:text-gray-400 bg-gray-50/80 dark:bg-gray-900/40 rounded-b-lg">
                              <span className="font-medium text-gray-700 dark:text-gray-300">Unidades (referencia): </span>
                              {row.unidad_labels!.join(', ')}
                            </td>
                          </tr>
                        )}
                        </>

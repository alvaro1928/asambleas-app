...
export async function GET() {
  try {
    ...
    const { data: perfiles } = await admin
      .from('profiles')
      .select('user_id')
    const userIds = new Set((perfiles ?? []).map((p: { user_id?: string }) => p.user_id).filter(Boolean))
    ...
    const ranking: Array<{ user_id: string; email: string | null; full_name: string | null; tokens_disponibles: number }> = []

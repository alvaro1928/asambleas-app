...
/**
 * POST /api/delegado/participacion-timer/set-default
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: updateError } = await admin
      .from('asambleas')
      .update({ participacion_timer_default_minutes: minutesNum })
      .eq('id', asambleaId)

    if (updateError) return NextResponse.json({ error: 'Error al actualizar default' }, { status: 500 })

    return NextResponse.json({ ok: true, participacion_timer_default_minutes: minutesNum })
  } catch (e) {
    console.error('participacion-timer/set-default (delegado):', e)

-- =====================================================
-- SCRIPT FINAL: VOTACIÓN SEGÚN LEY 675
-- =====================================================
-- Este es el script definitivo que incluye TODO:
-- 1. Deshabilita RLS
-- 2. Crea/actualiza tablas
-- 3. Función de registro de votos
-- 4. Función de estadísticas MEJORADA (Ley 675)
-- =====================================================
...
RETURNS TABLE (voto_id UUID, accion TEXT, mensaje TEXT) AS $$
DECLARE
...
BEGIN
  ...
  IF v_voto_existente IS NOT NULL THEN
    v_accion := 'modificar';
    UPDATE votos SET opcion_id = p_opcion_id, votante_email = p_votante_email, 
      votante_nombre = p_votante_nombre, es_poder = p_es_poder, poder_id = p_poder_id
  ...
  INSERT INTO historial_votos (voto_id, pregunta_id, unidad_id, opcion_id, votante_email, votante_nombre, 
    es_poder, poder_id, accion, opcion_anterior_id, ip_address, user_agent)
  VALUES (v_nuevo_voto_id, p_pregunta_id, p_unidad_id, p_opcion_id, p_votante_email, p_votante_nombre, 
    p_es_poder, p_poder_id, v_accion, v_opcion_anterior, p_ip_address, p_user_agent);

  RETURN QUERY SELECT v_nuevo_voto_id, v_accion,
    CASE WHEN v_accion = 'crear' THEN 'Voto registrado' ELSE 'Voto actualizado' END;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- =====================================================
-- FUNCIÓN ESTADÍSTICAS LEY 675
-- =====================================================
...
BEGIN
  ...
  SELECT JSONB_AGG(
    JSONB_BUILD_OBJECT(
      ...
      'porcentaje_votos_emitidos', COALESCE(
        CASE WHEN v_total_votos > 0 THEN ROUND((stats.votos_count::NUMERIC / v_total_votos * 100), 2) ELSE 0 END, 0
      ),
      'porcentaje_coeficiente_emitido', COALESCE(
        CASE WHEN v_total_coeficiente > 0 THEN ROUND((stats.votos_coeficiente / v_total_coeficiente * 100), 2) ELSE 0 END, 0
      ),
      'porcentaje_coeficiente_total', COALESCE(
        CASE WHEN v_coeficiente_conjunto > 0 THEN ROUND((stats.votos_coeficiente / v_coeficiente_conjunto * 100), 2) ELSE 0 END, 0
      )
    ) ORDER BY op.orden
  ) INTO v_resultados
  FROM opciones_pregunta op
  LEFT JOIN (
    SELECT v.opcion_id, COUNT(v.id)::INTEGER AS votos_count, SUM(u.coeficiente)::NUMERIC(12, 6) AS votos_coeficiente
    FROM votos v
    JOIN unidades u ON v.unidad_id = u.id
    WHERE v.pregunta_id = p_pregunta_id
  ...
  RETURN QUERY SELECT v_total_votos, v_total_coeficiente, v_coeficiente_conjunto,
    CASE WHEN v_coeficiente_conjunto > 0 THEN ROUND((v_total_coeficiente / v_coeficiente_conjunto * 100), 2) ELSE 0 END,
    v_resultados;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Verificación
SELECT '✅ TODO LISTO CON LEY 675' as status;

SELECT proname FROM pg_proc WHERE proname IN ('registrar_voto_con_trazabilidad', 'calcular_estadisticas_pregunta');
